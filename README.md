...
— troque pra senha que você cadastrou em `/setup`).

## Como começar

Três formas de rodar, em ordem de feature-completude:
...
As três rotas partem do mesmo código — muda só o
empacotamento. Detalhes nas seções abaixo.
...
A camada de **domínio** (`domain/games/LaunchOptions`,
`ConfigCatalog`, `CompatibilityRules`) é agnóstica de loja —
trabalha com a string de launch options do Proton (env vars + args +
gamescope), que é igual para qualquer prefix Proton, venha o jogo
da Steam, Epic ou GOG.

O que está acoplado a Steam hoje:

- **Sync de biblioteca** — chama o skill `steam-launch` (Steam Web API).
- **Apply direto** — escreve no `localconfig.vdf` do Steam.
- **/check** — usa Steam Search + ProtonDB (que é indexado por Steam appid).

Pra cobrir Epic Games (via Heroic / Legendary) ou GOG:

1. **Novo outbound adapter** `EpicLibraryClient` (lê
...
2. **Coluna `store` em `games`** — `'steam' | 'epic' | 'gog'`. AppID
   passa a ser `<store>:<id>` pra não colidir.
3. **Apply direto** — novo `HeroicLocalConfigFs` que edita
   `~/.config/heroic/GamesConfig/<id>.json`.
4. **/check** — Epic não tem ProtonDB próprio; estratégia
   possível: tentar match por nome no Steam search e usar o appid
   Steam como proxy do tier ProtonDB. PCGamingWiki funciona sem mudança
   (é indexada por nome de jogo, não por loja).

Os widgets de "Editor de launch options", "PCGW widescreen" e o
"Wizard de pacotes" funcionariam sem mudança alguma — só
a origem e o destino do `user_launch_options` mudam.

Não implementado ainda; contribuições bem-vindas.
