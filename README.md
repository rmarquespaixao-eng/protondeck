...
Script usa Playwright + Chromium headless. Faz login com o user/senha
exportados em `PD_USER` / `PD_PASSWORD` (defaults `admin` / `changeme`
— troque pra senha que voc&ecirc; cadastrou em `/setup`).

## Como come&ccedil;ar

Tr&ecirc;s formas de rodar, em ordem de feature-completude:

| Modo                          | Quando usar                                              | Como                                              |
|-------------------------------|----------------------------------------------------------|---------------------------------------------------|
| **Tarball + systemd (host)**  | uso real no PC gamer Linux — todas as features          | `npm run release` → [`./install.sh`](#tarball-recomendado-pra-uso-completo) |
| **Dev (`npm run dev`)**       | desenvolvendo / contribuindo                             | [`npm install` + `npm run dev`](#setup-desenvolvimento) |
| **Docker**                    | expor o painel em servidor remoto (sem wizard nem apply Steam) | [`docker compose up -d`](#docker)                |

As tr&ecirc;s rotas partem do mesmo c&oacute;digo — muda s&oacute; o
empacotamento. Detalhes nas se&ccedil;&otilde;es abaixo.
...
```bash
git clone https://github.com/rmarquespaixao-eng/protondeck.git
cd protondeck
npm install
cp .env.example .env
...
## Roadmap — outras lojas (Epic, GOG)

A camada de **dom&iacute;nio** (`domain/games/LaunchOptions`,
`ConfigCatalog`, `CompatibilityRules`) &eacute; agn&oacute;stica de loja —
trabalha com a string de launch options do Proton (env vars + args +
gamescope), que &eacute; igual para qualquer prefix Proton, venha o jogo
da Steam, Epic ou GOG.

O que est&aacute; acoplado a Steam hoje:

- **Sync de biblioteca** — chama o skill `steam-launch` (Steam Web API).
- **Apply direto** — escreve no `localconfig.vdf` do Steam.
- **/check** — usa Steam Search + ProtonDB (que &eacute; indexado por Steam appid).

Pra cobrir Epic Games (via Heroic / Legendary) ou GOG:

1. **Novo outbound adapter** `EpicLibraryClient` (l&ecirc;
   `~/.config/heroic/store_cache/legendary_library.json` ou roda o CLI
   `legendary list-installed --json`).
2. **Coluna `store` em `games`** — `'steam' | 'epic' | 'gog'`. AppID
   passa a ser `<store>:<id>` pra n&atilde;o colidir.
3. **Apply direto** — novo `HeroicLocalConfigFs` que edita
   `~/.config/heroic/GamesConfig/<id>.json`.
4. **/check** — Epic n&atilde;o tem ProtonDB pr&oacute;prio; estrat&eacute;gia
   poss&iacute;vel: tentar match por nome no Steam search e usar o appid
   Steam como proxy do tier ProtonDB. PCGamingWiki funciona sem mudan&ccedil;a
   (&eacute; indexada por nome de jogo, n&atilde;o por loja).

Os widgets de "Editor de launch options", "PCGW widescreen" e o
"Wizard de pacotes" funcionariam sem mudan&ccedil;a alguma — s&oacute;
a origem e o destino do `user_launch_options` mudam.

N&atilde;o implementado ainda; contribui&ccedil;&otilde;es bem-vindas.
